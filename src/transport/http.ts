...
async function createNewSession(
    ...
    const transport = new StreamableHTTPServerTransport({
        ...
        onsessioninitialized: (sessionId) => {
            ...
            console.log('New Photon iMessage session created:', sessionId);
        }
    });
    ...
    transport.onclose = () => {
        if (transport.sessionId) {
            ...
            console.log('Photon iMessage session closed:', transport.sessionId);
        }
    };
...
function logServerStart(config: Config): void {
    ...
    console.log(`Photon iMessage MCP Server listening on ${displayUrl}`);
